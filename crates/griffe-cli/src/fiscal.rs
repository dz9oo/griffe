...
use griffe_core::fiscal::{FiscalDeadline, fiscal_calendar, upcoming_deadlines};
...
pub enum FiscalCommand {
    ...
    /// indicatives (année civile), voir `griffe_core::fiscal`.
    Deadlines {
...
fn to_json(d: &FiscalDeadline) -> serde_json::Value {
    json!({
        ...
        "amount_cents": d.amount.map(griffe_core::domain::Money::cents),
        "note": d.note,
    })
}
