...
//! approbation, documents de clôture (`griffe-docs`).
//!
//! Depuis le lot 31, `year balance` et `year render … balance-sheet` exposent le grand livre
//! dérivé (`griffe_core::ledger`) : balance des comptes et bilan 2033-A, pour un exercice clos
...
//! (`griffe_core::closing`) : les étapes viennent du cœur, seule l'*action* suggérée pour
...
use griffe_core::domain::{FiscalYearEnd, Money, OpeningBalanceLine, format_date};
...
pub enum OpeningCommand {
    ...
    Import {
        ...
        #[arg(long, value_parser = clap::value_parser!(griffe_core::opening_balance::import::OpeningImportFormat))]
        format: Option<griffe_core::opening_balance::import::OpeningImportFormat>,
...
) -> Result<griffe_core::app::Outcome<i64>, CliError> {
...
fn import_preview_human(preview: &griffe_core::opening_balance::import::ImportPreview) -> String {
    let rows: Vec<Vec<String>> = preview
        .lines
        .iter()
        .map(|l| {
            let (debit, credit) = match l.side {
                griffe_core::domain::Side::Debit => (l.amount.to_string(), String::new()),
                griffe_core::domain::Side::Credit => (String::new(), l.amount.to_string()),
            };
    ...
    let mut out = format!(
        ...
        match preview.format {
            griffe_core::opening_balance::import::OpeningImportFormat::Balance => {
                "une balance générale"
            }
            griffe_core::opening_balance::import::OpeningImportFormat::Fec => "un FEC",
            griffe_core::opening_balance::import::OpeningImportFormat::Cerfa2033A => {
                "un 2033-A"
            }
        },
        preview.lines.len(),
...
fn run_opening(
    ...
    let output = match cmd {
        ...
        OpeningCommand::Import {
            ...
            let preview = griffe_core::opening_balance::import::import_opening_balance(
                &bytes, opens_on, format,
            )
            .map_err(|e| CliError::Domain(e.to_string()))?;
            if ctx.dry_run {
                return Ok(if json {
                    format_json(&griffe_core::opening_balance::import::preview_json(
                        &preview,
                    ))
                } else {
            ...
            if let Some(months) = duration
                && matches!(outcome, griffe_core::app::Outcome::Applied(_))
            {
        ...
        OpeningCommand::From2033a {
            ...
            let mapped: griffe_core::opening_balance::import::CerfaBoxes =
                boxes.into_iter().collect();
            let preview = griffe_core::opening_balance::import::from_2033a(opens_on, &mapped)
                .map_err(|e| CliError::Domain(e.to_string()))?;
            if ctx.dry_run {
                return Ok(if json {
                    format_json(&griffe_core::opening_balance::import::preview_json(
...
pub(crate) fn pre_approve_backup_for_action(
    ...
    let period = griffe_core::fiscal_year::fiscal_year_by_id(store.connection(), cmd.id)?
...
pub(crate) fn render_bytes(
    ...
    if matches!(doc, DocKind::BalanceSheet | DocKind::Inventory) {
        ...
        return match doc {
            DocKind::BalanceSheet => griffe_docs::render_balance_sheet(
                &profile,
                &ledger.balance_sheet(),
                &ledger.trial_balance(),
            ),
            DocKind::Inventory => griffe_docs::render_inventory(&profile, &ledger.trial_balance()),
            DocKind::Minutes
    ...
    match doc {
        ...
        DocKind::Appropriation => griffe_docs::render_appropriation_decision(&profile, &record)
            .map_err(|e| CliError::Unexpected(e.to_string())),
        DocKind::Synthesis => {
        ...
        DocKind::EfiNotice => {
            let ledger = build_ledger(store.connection(), record.period())?;
            let export = griffe_docs::liasse_export(&profile, &record, Some(&ledger));
            griffe_docs::render_efi_notice(&export).map_err(|e| CliError::Unexpected(e.to_string()))
        }
    }
}
...
pub fn run(
    ...
    let output = match cmd {
        ...
        YearCommand::Approve {
            ...
            let note = if matches!(outcome, griffe_core::app::Outcome::Applied(_)) {
