...
//!
//! Le cœur construit et rend le fichier (`griffe_core::fec`) ; l'écriture sur disque est de
...
//! [`griffe_core::fec::check_fec`].

use std::fmt::Write as _;
use std::path::{Path, PathBuf};

use clap::Subcommand;
use griffe_core::app::ExecutionContext;
use griffe_core::fec::{FecCheck, FecSeverity, build_fec, check_fec, check_fec_of};
            ...
            if json {
                ...
                Ok(format!(
                    "✓ {} écrit — exercice {} → {}, {} écriture(s), {} ligne(s), total débit = \
                     total crédit = {}",
                    path.display(),
                    griffe_core::domain::format_date(summary.starts_on),
                    griffe_core::domain::format_date(summary.ends_on),
                    summary.entries,
                    summary.lines,
                    fec.total_debit()
                ))
