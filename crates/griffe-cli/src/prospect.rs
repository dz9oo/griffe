...
//! casse et aux accents, exact puis par préfixe) : voir `griffe_core::reference`.

use clap::Subcommand;
use griffe_core::app::{ExecutionContext, Executor};
use griffe_core::domain::{Address, InteractionId, InteractionKind, OpportunityStage, Probability};
...
impl HumanRender for griffe_core::domain::Opportunity {
    fn render_human(&self) -> String {
        ...
        key_values(&[
            ...
            (
                "prochaine action",
                or_dash(self.next_action_at.map(griffe_core::domain::format_date)),
            ),
...
impl HumanRender for griffe_core::prospection::OpportunityReferences {
    fn render_human(&self) -> String {
        format!(
            "{} devis, {} mission(s){}",
            self.quotes,
...
impl HumanRender for Vec<griffe_core::domain::Opportunity> {
    fn render_human(&self) -> String {
...
pub enum ProspectCommand {
    ...
    Create {
        ...
        amount: griffe_core::domain::Money,
    ...
    Edit {
        ...
        name: Option<String>,
        #[arg(long, value_parser = parse_money)]
        amount: Option<griffe_core::domain::Money>,
        #[arg(long, value_parser = parse_probability)]
    ...
    Lose {
        ...
        reason: griffe_core::domain::LossReason,
    },
...
fn opportunity_table(opportunities: &[griffe_core::domain::Opportunity]) -> String {
    let rows = opportunities
        ...
        .map(|o| {
            vec![
                ...
                o.next_action_at
                    .map_or_else(|| "—".to_string(), griffe_core::domain::format_date),
...
fn interaction_table(interactions: &[griffe_core::domain::Interaction]) -> String {
    ...
    let output = match cmd {
        ...
        ProspectCommand::Edit {
            ...
            let outcome = if party_touched {
                let party =
                    griffe_core::clients::client_by_id(store.connection(), current.client_id)?
                        .ok_or_else(|| {
...
fn interaction_or_not_found(
    store: &Store,
    id: InteractionId,
) -> Result<griffe_core::domain::Interaction, CliError> {
