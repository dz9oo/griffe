...
//! `griffe` (ce crate) empruntent exactement le même chemin de code — [`run`] imprime sur
...
enum TopCommand {
    ...
    /// Ne crée jamais le coffre (utilisez `griffe init`).
    Unlock,
    ...
    Confirm {
        /// Identifiant de l'action (voir `griffe pending list`).
...
fn dispatch(cli: Cli, access: VaultAccess<'_>) -> Result<String, CliError> {
    ...
    match access {
        ...
        VaultAccess::Process => {
            ...
            match griffe_core::receipts::migrate_legacy(&store) {
                Ok(0) => {}
                Ok(n) => eprintln!(
