...
//! aux accents, exact puis par préfixe) : voir `griffe_core::reference`.

use clap::{Args, Subcommand};
use griffe_core::app::{ExecutionContext, Executor};
use griffe_core::domain::{Milestone, MissionKind, Money, QuoteId, TimeCategory, TimeEntryId};
use griffe_core::missions::{
...
impl HumanRender for griffe_core::domain::Mission {
    fn render_human(&self) -> String {
        ...
        key_values(&[
            ...
            (
                "démarrée",
                griffe_core::domain::format_date(self.started_on),
            ),
            (
                "terminée",
                or_dash(self.ended_on.map(griffe_core::domain::format_date)),
            ),
...
impl HumanRender for griffe_core::missions::MissionReferences {
    fn render_human(&self) -> String {
        format!(
            ...
            let rows: Vec<Vec<String>> = installments
                ...
                .map(|(m, amount)| {
                    vec![
                        ...
                        or_dash(m.due_on.map(griffe_core::domain::format_date)),
                        amount.to_string(),
                    ]
                })
                .collect();
...
fn parse_month(s: &str) -> Result<griffe_core::domain::Month, CliError> {
    ...
    let month: u8 = month_str
        .parse()
        .map_err(|_| CliError::Domain(format!("mois invalide : {s}")))?;
    griffe_core::domain::Month::new(year, month).map_err(|e| CliError::Domain(e.to_string()))
}

fn mission_or_not_found(
    store: &Store,
    reference: &str,
) -> Result<griffe_core::domain::Mission, CliError> {
...
fn time_entry_or_not_found(
    store: &Store,
    id: TimeEntryId,
) -> Result<griffe_core::domain::TimeEntry, CliError> {
    time_entry_by_id(store.connection(), id)?
        .ok_or_else(|| CliError::Domain(format!("saisie de temps introuvable : {id}")))
}

fn mission_table(missions: &[griffe_core::domain::Mission]) -> String {
    let rows = missions
        .iter()
        .map(|m| {
            ...
            vec![
                ...
                m.name.clone(),
                kind.to_string(),
                griffe_core::domain::format_date(m.started_on),
                status,
            ]
        })
        .collect::<Vec<_>>();
...
fn time_entry_table(entries: &[griffe_core::domain::TimeEntry]) -> String {
    let rows = entries
        .iter()
        .map(|e| {
            vec![
                e.id.to_string(),
                griffe_core::domain::format_date(e.worked_on),
                e.days.to_string(),
...
pub fn run(
    ...
    let output = match cmd {
        ...
        MissionCommand::Rate { reference } => {
            ...
            if json {
                format_json(&rate.map(griffe_core::domain::Money::cents))
            } else {
                rate.map_or_else(
