...
use griffe_core::app::{Executor, Outcome};
...
async fn deleting_a_client_still_referenced_by_an_opportunity_is_refused_at_confirmation() {
    ...
    assert_eq!(body["status"], "pending_confirmation");
    let pending_id: griffe_core::app::PendingActionId =
        body["pending_action_id"].as_str().unwrap().parse().unwrap();

    let mut confirming_store =
        Store::open_with_passphrase(&db_path, &Passphrase::from("s3cret")).unwrap();
    let err = Executor::new(&mut confirming_store)
        .confirm::<griffe_core::clients::DeleteClient>(pending_id)
        .unwrap_err();
    assert!(
...
async fn reading_the_clients_collection_resource_lists_active_clients() {
    ...
    assert!(resources.iter().any(|r| r.uri == "griffe://clients"));

    let read = client
        .read_resource(ReadResourceRequestParams::new("griffe://clients"))
        .await
        .unwrap();
...
async fn reading_the_missions_resource_returns_the_active_missions() {
    ...
    let resources = client.list_resources(None).await.unwrap().resources;
    assert!(resources.iter().any(|r| r.uri == "griffe://missions"));

    let read = client
        .read_resource(ReadResourceRequestParams::new("griffe://missions"))
        .await
        .unwrap();
    let text = match &read.contents[0] {
        rmcp::model::ResourceContents::TextResourceContents { text, .. } => text.clone(),
...
async fn reading_an_unknown_resource_uri_is_a_protocol_level_error() {
    ...
    let result = client
        .read_resource(ReadResourceRequestParams::new("griffe://unknown"))
        .await;
...
fn set_company_profile(store: &mut Store) {
    let cmd = griffe_core::company::SetCompanyProfile {
    ...
    Executor::new(store)
        .execute(
            &cmd,
            &griffe_core::app::ExecutionContext::new(griffe_core::app::Actor::Human, false),
        )
        .unwrap();
}
...
async fn an_agent_closing_a_fiscal_year_only_deposits_a_pending_action() {
    ...
    let read = client
        .read_resource(ReadResourceRequestParams::new("griffe://fiscal-years"))
        .await
        .unwrap();
    let text = match &read.contents[0] {
        rmcp::model::ResourceContents::TextResourceContents { text, .. } => text.clone(),
...
async fn expense_lifecycle_over_mcp() {
    ...
    let mut confirming_store =
        Store::open_with_passphrase(&db_path, &Passphrase::from("s3cret")).unwrap();
    let confirmed = Executor::new(&mut confirming_store)
        .confirm::<griffe_core::expenses::DeleteExpense>(pending_id.parse().unwrap())
        .unwrap();
    assert!(matches!(confirmed, Outcome::Applied(())));
...
async fn expense_reconciliation_over_mcp_needs_a_human() {
    ...
    let mut confirming_store =
        Store::open_with_passphrase(&db_path, &Passphrase::from("s3cret")).unwrap();
    let confirmed = Executor::new(&mut confirming_store)
        .confirm::<griffe_core::expenses::RecordExpense>(pending_id.parse().unwrap())
        .unwrap();
    ...
    Executor::new(&mut confirming_store)
        .confirm::<griffe_core::expenses::ReconcileExpense>(pending_id.parse().unwrap())
        .unwrap();
    drop(confirming_store);
    let listed = call(&client, "bank.list", json!({"unmatched": true})).await;
    ...
    let resource = client
        .read_resource(ReadResourceRequestParams::new("griffe://expenses/frais"))
        .await
        .unwrap();
...
async fn a_quote_is_readable_by_tool_and_resource_after_creation() {
    ...
    let resource = client
        .read_resource(ReadResourceRequestParams::new("griffe://quotes"))
        .await
        .unwrap();
    let rmcp::model::ResourceContents::TextResourceContents { text, .. } = &resource.contents[0]
    else {
...
async fn a_payment_correction_proposed_by_an_agent_waits_for_a_human() {
    ...
    let mut store = Store::create(&db_path, &Passphrase::from("s3cret")).unwrap();
    let human = griffe_core::app::ExecutionContext::new(griffe_core::app::Actor::Human, false);

    // Fixture posée en acteur humain, avant de donner le coffre au serveur : une facture payée.
    let client_id = griffe_core::domain::ClientId::new();
    store
        .connection()
        .execute(
        ...
        .execute(
            &EmitInvoice {
                ...
                lines: vec![griffe_core::domain::InvoiceLine {
                    ...
                    quantity: 10.0,
                    unit_price: griffe_core::domain::Money::from_cents(65_000),
    ...
    Executor::new(&mut store)
        .execute(
            &RecordPayment {
                ...
                amount: griffe_core::domain::Money::from_cents(780_000),
    ...
    let mut confirming_store =
        Store::open_with_passphrase(&db_path, &Passphrase::from("s3cret")).unwrap();
    let confirmed = Executor::new(&mut confirming_store)
        .confirm::<griffe_core::billing::VoidPayment>(pending_id.parse().unwrap())
        .unwrap();
    assert!(matches!(confirmed, Outcome::Applied(())));
...
async fn the_company_profile_roundtrips_through_set_profile_show_and_the_company_resource() {
    ...
    let read = client
        .read_resource(ReadResourceRequestParams::new("griffe://company"))
        .await
        .unwrap();
    let text = match &read.contents[0] {
        rmcp::model::ResourceContents::TextResourceContents { text, .. } => text.clone(),
...
async fn a_fiscal_year_can_be_shown_and_amended_but_approval_and_deletion_need_a_human() {
    ...
    use griffe_core::company::SetCompanyProfile;
    use griffe_core::domain::{Address, Money, Siren};
    use griffe_core::fiscal_year::CloseFiscalYear;
    ...
    let glossary = client
        .read_resource(ReadResourceRequestParams::new("griffe://closing-glossary"))
        .await
        .unwrap();
        ...
        .execute(
            &EmitInvoice {
                ...
                lines: vec![griffe_core::domain::InvoiceLine {
                    description: "Prestation".to_string(),
                    quantity: 2.0,
                    unit_price: griffe_core::domain::Money::from_cents(100_000),
                    vat_rate: griffe_core::domain::VatRate::Standard,
                }],
...
async fn an_agent_setting_the_opening_balance_only_deposits_a_pending_action() {
    ...
    let read = client
        .read_resource(ReadResourceRequestParams::new("griffe://opening-balance"))
        .await
        .unwrap();
    let text = match &read.contents[0] {
        rmcp::model::ResourceContents::TextResourceContents { text, .. } => text.clone(),
...
async fn settling_a_balance_sheet_account_over_mcp_needs_a_human() {
    use griffe_core::app::{Actor, ExecutionContext};
    use griffe_core::billing::{ImportBankTransactions, ParsedTransaction, SettleBankTransaction};
...
async fn bank_import_over_mcp_detects_the_format_and_reports_what_it_understood() {
    ...
    let latin1 = include_bytes!("../../griffe-core/src/billing/fixtures/credit-agricole.csv");
    ...
    Executor::new(&mut store)
        .execute(
            &RecordExpense {
                ...
                amount: griffe_core::domain::Money::from_cents(12_000),
                vat_rate: griffe_core::domain::VatRate::Standard,
                vat_deductible: griffe_core::domain::Money::from_cents(2_000),
...
async fn importing_an_opening_balance_over_mcp_previews_then_needs_a_human() {
    ...
    let fec = String::from_utf8_lossy(include_bytes!(
        "../../griffe-core/src/opening_balance/fixtures/fec-tiime.txt"
    ))
    .into_owned();
    let preview = json_of(
        &call(
...
async fn papers_add_over_mcp_is_direct_and_purge_needs_a_human() {
    ...
    let resources = client.list_resources(None).await.unwrap().resources;
    assert!(resources.iter().any(|r| r.uri == "griffe://papers"));
    let read = client
        .read_resource(ReadResourceRequestParams::new("griffe://papers"))
        .await
        .unwrap();
    let text = match &read.contents[0] {
        rmcp::model::ResourceContents::TextResourceContents { text, .. } => text.clone(),
    ...
    let period_res = client
        .read_resource(ReadResourceRequestParams::new("griffe://papers/2026"))
        .await
        .unwrap();
...
async fn requesting_a_vat_refund_over_mcp_needs_a_human_before_case_26_appears() {
    use griffe_core::app::{Actor, ExecutionContext};
    use griffe_core::domain::{Address, FiscalYearEnd, Money, Siren, VatRegime};
    use griffe_core::society::{RecordVatCarryIn, RequestVatRefund};
...
async fn recording_a_vat_reversal_over_mcp_needs_a_human_before_case_15_appears() {
    use griffe_core::app::{Actor, ExecutionContext};
    use griffe_core::domain::{Address, FiscalYearEnd, Money, Siren, VatRegime};
    use griffe_core::society::{RecordVatCarryIn, RecordVatReversal};
