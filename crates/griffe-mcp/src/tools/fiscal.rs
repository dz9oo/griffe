...
use griffe_core::closing::{checklist_json, closing_checklist};
use griffe_core::company::{CompanyProfile, company_profile};
use griffe_core::domain::{FiscalYearEnd, Money, format_date};
use griffe_core::fiscal::{fiscal_calendar, upcoming_deadlines};
...
use griffe_core::ledger::{balance_json, build_ledger, ledger_ending_in};
...
impl FreeflowServer {
    ...
    async fn fiscal_set_opening_balance(
        ...
        let opens_on = ok_or_return!("opens_on", griffe_core::domain::parse_date(&args.opens_on));
        let mut lines = Vec::with_capacity(args.lines.len());
        for spec in &args.lines {
            lines.push(ok_or_return!(
                "lines",
                spec.parse::<griffe_core::domain::OpeningBalanceLine>()
            ));
        }
    ...
    async fn fiscal_import_opening_balance(
        ...
        let preview = if let Some(raw_boxes) = args.boxes.as_ref() {
            let mut mapped = griffe_core::opening_balance::import::CerfaBoxes::new();
            ...
            ok_or_return!(
                "boxes",
                griffe_core::opening_balance::import::from_2033a(opens_on, &mapped)
            )
        } else {
            ...
            let hint = match args.format.as_deref() {
                ...
                Some(raw) => Some(ok_or_return!(
                    "format",
                    raw.parse::<griffe_core::opening_balance::import::OpeningImportFormat>()
                )),
            };
            ok_or_return!(
                "content",
                griffe_core::opening_balance::import::import_opening_balance(
                    &bytes, opens_on, hint
                )
            )
        };
        if args.dry_run {
            return ok_json(griffe_core::opening_balance::import::preview_json(&preview));
        }
        let mut store = self.store.lock().await;
        let existing = match opening_balance(store.connection()) {
            Ok(existing) => existing,
        ...
        match result {
            Ok(mut body) => {
                body["preview"] = griffe_core::opening_balance::import::preview_json(&preview);
                if body["status"] == "applied"
                    && let Some(months) = args.duration_months
                {
                    let candidates = griffe_core::domain::fixed_asset_candidates(&preview.lines);
                    let mut declared = Vec::new();
                    for candidate in &candidates {
                        let cmd = griffe_core::fixed_assets::AddFixedAsset::from_candidate(
    ...
    /// dépôt au greffe, DSN) — dates et montants indicatifs, voir `griffe_core::fiscal`.
    #[tool(
    ...
    /// ancien au plus récent — voir aussi la ressource `griffe://fiscal-years`.
    #[tool(
        name = "fiscal.years",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    async fn fiscal_close_year(
        ...
        let (starts_on, ends_on): (Date, Date) = match (args.period, &args.starts_on, &args.ends_on)
        {
            ...
            (None, Some(start), Some(end)) => (
                ok_or_return!("starts_on", griffe_core::domain::parse_date(start)),
                ok_or_return!("ends_on", griffe_core::domain::parse_date(end)),
            ),
            _ => {
                return err_text(
        ...
        let today = match args.today.as_deref() {
            None => griffe_core::clock::today_local(),
            Some(raw) => ok_or_return!("today", griffe_core::domain::parse_date(raw)),
        };
    ...
    async fn fiscal_approve_year(
        ...
        let approved_on: Date = ok_or_return!(
            "approved_on",
            griffe_core::domain::parse_date(&args.approved_on)
        );
        let mut store = self.store.lock().await;
        let record = match require_year(&store, args.period) {
            Ok(r) => r,
    ...
    /// `griffe://closing-glossary`, à reprendre auprès d'un utilisateur non comptable.
    #[tool(
    ...
    async fn fiscal_render_year(
        ...
        if args.doc == "balance_sheet" || args.doc == "inventory" {
            ...
            let bytes = if args.doc == "inventory" {
                ok_or_return!(
                    "inventory",
                    griffe_docs::render_inventory(&profile, &ledger.trial_balance())
                )
            } else {
                ok_or_return!(
                    "balance_sheet",
                    griffe_docs::render_balance_sheet(
                        &profile,
                        &ledger.balance_sheet(),
                        &ledger.trial_balance()
                    )
            ...
            let _ = griffe_cli::capture_year(&mut store, &self.ctx(false), args.period);
        ...
        let bytes = match args.doc.as_str() {
            "minutes" => {
                let today: Option<Date> = match &args.today {
                    Some(s) => Some(ok_or_return!("today", griffe_core::domain::parse_date(s))),
                    None => None,
                };
                ...
                ok_or_return!(
                    "minutes",
                    griffe_docs::render_approval_minutes(&profile, &record, today)
                )
            }
            "appropriation" => ok_or_return!(
                "appropriation",
                griffe_docs::render_appropriation_decision(&profile, &record)
            ),
            "synthesis" => {
                ...
                ok_or_return!(
                    "synthesis",
                    griffe_docs::render_synthesis(&profile, &result, prior)
                )
            }
            "liasse" => {
                let ledger =
                    ok_or_return!("ledger", build_ledger(store.connection(), record.period()));
                let export = griffe_docs::liasse_export(&profile, &record, Some(&ledger));
            ...
            "efi_notice" => {
                let ledger =
                    ok_or_return!("ledger", build_ledger(store.connection(), record.period()));
                let export = griffe_docs::liasse_export(&profile, &record, Some(&ledger));
                ok_or_return!("efi_notice", griffe_docs::render_efi_notice(&export))
            }
            other => {
                return err_text(format!(
    ...
    async fn fiscal_assets(&self, Parameters(args): Parameters<AssetsListArgs>) -> CallToolResult {
        ...
        let fy = match args.period {
            ...
            None => fye.containing(griffe_core::clock::today_local()),
        };
        match griffe_core::fixed_assets::list_fixed_assets(store.connection()) {
            Ok(assets) => ok_json(
                assets
                    .iter()
    ...
    async fn fiscal_add_asset(&self, Parameters(args): Parameters<AddAssetArgs>) -> CallToolResult {
        let acquired_on = ok_or_return!(
            "acquired_on",
            griffe_core::domain::parse_date(&args.acquired_on)
        );
        let account = ok_or_return!(
            "account",
            args.account.parse::<griffe_core::domain::AccountCode>()
        );
        let mut store = self.store.lock().await;
        ...
        let cmd = griffe_core::fixed_assets::AddFixedAsset {
            label: args.label,
            account,
            acquired_on,
    ...
    async fn fiscal_delete_asset(
        ...
        let asset = match griffe_core::fixed_assets::fixed_asset_by_id(store.connection(), id) {
            Ok(Some(a)) => a,
            Ok(None) => return err_text("immobilisation introuvable"),
            Err(e) => return err_text(e.to_string()),
        };
        let cmd = griffe_core::fixed_assets::DeleteFixedAsset {
            id,
