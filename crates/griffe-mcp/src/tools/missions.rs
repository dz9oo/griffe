...
fn parse_month(s: &str) -> Result<griffe_core::domain::Month, String> {
...
fn mission_or_not_found(
    ...
    id: MissionId,
) -> Result<griffe_core::domain::Mission, CallToolResult> {
...
impl FreeflowServer {
    ...
    async fn mission_update(
        ...
        let started_on = match &args.started_on {
            Some(s) => ok_or_return!("started_on", griffe_core::domain::parse_date(s)),
    ...
    async fn mission_close(
        ...
        let ended_on = ok_or_return!("ended_on", griffe_core::domain::parse_date(&args.ended_on));
    ...
    async fn mission_log_time(&self, Parameters(args): Parameters<LogTimeArgs>) -> CallToolResult {
        ...
        let worked_on = ok_or_return!(
            "worked_on",
            griffe_core::domain::parse_date(&args.worked_on)
        );
    ...
    async fn mission_time_update(
        ...
        let worked_on = match &args.worked_on {
            Some(s) => ok_or_return!("worked_on", griffe_core::domain::parse_date(s)),
