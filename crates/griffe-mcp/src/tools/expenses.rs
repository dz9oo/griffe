...
//! de l'IO d'adaptateur au poste de l'utilisateur — voir `griffe-cli::expense::archive_receipt`
...
use griffe_core::domain::{BankTransactionId, ExpenseCategory, ExpensePaidBy, Money, VatRate};
use griffe_core::expenses::{self, expense_by_id, expense_detail, list_expenses};
...
impl FreeflowServer {
    ...
    async fn expense_update(
        ...
        let incurred_on = match &args.incurred_on {
            Some(s) => ok_or_return!("incurred_on", griffe_core::domain::parse_date(s)),
    ...
    async fn expense_attach_receipt(
        ...
        match Executor::new(&mut store).execute(&cmd, &self.ctx(args.dry_run)) {
            Ok(outcome) => {
                ...
                {
                    let _ = griffe_cli::capture_expense_receipt(
                        &mut store,
                        &self.ctx(false),
                        id,
                        &receipt_filename,
