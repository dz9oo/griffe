...
use griffe_core::domain::{PaperKind, PaperOrigin, parse_date};
use griffe_core::expenses::hash_receipt;
...
impl FreeflowServer {
    ...
    /// Lecture seule. `today` facultatif. Voir aussi la ressource `griffe://papers/{period}`.
    #[tool(
    ...
    async fn papers_export(
        ...
        let store = self.store.lock().await;
        match griffe_cli::write_control_pack(
            &store,
            args.period,
