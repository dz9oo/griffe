...
//! `company.show` lit tout — voir aussi la ressource `griffe://company`.

use griffe_core::app::Executor;
use griffe_core::company;
use griffe_core::domain::{Address, FiscalYearEnd, Money, Siren, VatNumber, VatRegime};
use griffe_core::fiscal::company_profile_with_vat_filing;
...
/// `griffe_cli::company` pour la même raison que les résolveurs de `support` (format d'erreur
