//! Ressources MCP `griffe://clients` (les clients actifs) et `griffe://clients/{référence}`
...
use griffe_core::prospection::{self, OpportunityFilter};
...
const CLIENTS_COLLECTION_URI: &str = "griffe://clients";
const CLIENT_DETAIL_PREFIX: &str = "griffe://clients/";
const OPPORTUNITIES_COLLECTION_URI: &str = "griffe://opportunities";
const OPPORTUNITY_DETAIL_PREFIX: &str = "griffe://opportunities/";
const MISSIONS_COLLECTION_URI: &str = "griffe://missions";
const MISSION_DETAIL_PREFIX: &str = "griffe://missions/";
const QUOTES_COLLECTION_URI: &str = "griffe://quotes";
const QUOTE_DETAIL_PREFIX: &str = "griffe://quotes/";
const EXPENSES_COLLECTION_URI: &str = "griffe://expenses";
const EXPENSE_DETAIL_PREFIX: &str = "griffe://expenses/";
const FISCAL_YEARS_COLLECTION_URI: &str = "griffe://fiscal-years";
const COMPANY_URI: &str = "griffe://company";
const OPENING_BALANCE_URI: &str = "griffe://opening-balance";
const BALANCE_SHEET_PREFIX: &str = "griffe://balance-sheet/";
const CLOSING_CHECKLIST_PREFIX: &str = "griffe://closing-checklist/";
const CLOSING_GLOSSARY_URI: &str = "griffe://closing-glossary";
const ASSETS_URI: &str = "griffe://assets";
const FOLLOW_UPS_URI: &str = "griffe://follow-ups";
const DAY_URI: &str = "griffe://day";
const DAY_MONTH_PREFIX: &str = "griffe://day/month/";
const PEOPLE_URI: &str = "griffe://people";
const PEOPLE_DETAIL_PREFIX: &str = "griffe://people/";
const SOCIETY_URI: &str = "griffe://society";
const SOCIETY_CURRENT_ACCOUNT_URI: &str = "griffe://society/current-account";
const SOCIETY_DUTY_PREFIX: &str = "griffe://society/duties/";
const VAT_CARRY_URI: &str = "griffe://vat-carry-in";
const PAPERS_URI: &str = "griffe://papers";
const PAPERS_PERIOD_PREFIX: &str = "griffe://papers/";
...
pub(crate) fn list_templates() -> ListResourceTemplatesResult {
    ListResourceTemplatesResult::with_all_items(vec![
        ResourceTemplate::new(format!("{CLIENT_DETAIL_PREFIX}{{reference}}"), "client")
            .with_description(
                "Fiche d'un client (UUID, préfixe d'UUID, ou nom — voir griffe_core::reference), \
...
fn parse_month_uri(s: &str) -> Result<griffe_core::domain::Month, String> {
    ...
    let month: u8 = month_str
        .parse()
        .map_err(|_| format!("mois invalide : {s} (attendu AAAA-MM)"))?;
    griffe_core::domain::Month::new(year, month).map_err(|e| e.to_string())
}
...
pub(crate) fn read(store: &Store, uri: &str) -> Result<ReadResourceResult, McpError> {
    ...
    if let Some(reference) = uri.strip_prefix(MISSION_DETAIL_PREFIX) {
        ...
        let effective_daily_rate_cents = missions::effective_daily_rate(store.connection(), id)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?
            .map(griffe_core::domain::Money::cents);
    ...
    if uri == QUOTES_COLLECTION_URI {
        let quotes = griffe_core::quotes::list_quotes(store.connection())
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    if let Some(reference) = uri.strip_prefix(QUOTE_DETAIL_PREFIX) {
        let id = crate::support::resolve_quote(store, reference)
            .map_err(|e| McpError::resource_not_found(e, None))?;
        let quote = griffe_core::quotes::quote_by_id(store.connection(), id)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?
            .ok_or_else(|| {
                McpError::resource_not_found(format!("devis introuvable : {id}"), None)
            })?;
        let references = griffe_core::quotes::quote_references(store.connection(), id)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        let total_net_ht: griffe_core::domain::Money =
            griffe_core::quotes::priced_lines(&quote.lines, quote.discount)
                .iter()
    ...
    if uri == EXPENSES_COLLECTION_URI {
        let expenses = griffe_core::expenses::list_expenses(store.connection())
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    if let Some(reference) = uri.strip_prefix(EXPENSE_DETAIL_PREFIX) {
        let id = crate::support::resolve_expense(store, reference)
            .map_err(|e| McpError::resource_not_found(e, None))?;
        let expense = griffe_core::expenses::expense_detail(store.connection(), id)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?
            .ok_or_else(|| {
    ...
    if uri == FISCAL_YEARS_COLLECTION_URI {
        let years = griffe_core::fiscal_year::list_fiscal_years(store.connection())
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(
            uri,
            years
    ...
    if uri == CLOSING_GLOSSARY_URI {
        return json_contents(uri, griffe_core::closing::glossary_json());
    }

    if uri == FOLLOW_UPS_URI {
        let today = griffe_core::clock::today_local();
        let cards = griffe_core::follow_up::follow_up_queue(store.connection(), today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    if uri == DAY_URI {
        let today = griffe_core::clock::today_local();
        let mast = griffe_core::day::day_mast(store.connection(), today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        let gestures = griffe_core::day::day_gestures(store.connection(), today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(uri, json!({ "mast": mast, "gestures": gestures }));
    }
    if uri == SOCIETY_URI {
        let today = griffe_core::clock::today_local();
        let home = griffe_core::society::society_home(store.connection(), today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(uri, home);
    }
    if uri == SOCIETY_CURRENT_ACCOUNT_URI {
        let today = griffe_core::clock::today_local();
        let acc = griffe_core::society::current_account(store.connection(), today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(uri, acc);
    }
    if let Some(kind_raw) = uri.strip_prefix(SOCIETY_DUTY_PREFIX) {
        let kind = griffe_core::fiscal::FiscalDeadlineKind::parse(kind_raw).ok_or_else(|| {
            McpError::resource_not_found(format!("démarche inconnue : {kind_raw}"), None)
        })?;
        let today = griffe_core::clock::today_local();
        let briefing = griffe_core::society::duty_briefing(store.connection(), kind, today, None)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(uri, briefing);
    }
    if uri == PEOPLE_URI {
        let today = griffe_core::clock::today_local();
        let list = griffe_core::people::people_list(store.connection(), today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    if let Some(raw) = uri.strip_prefix(DAY_MONTH_PREFIX) {
        ...
        let view = griffe_core::day::day_month(store.connection(), month, today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    if uri == PAPERS_URI {
        let papers = griffe_core::papers::list_papers(
            store.connection(),
            griffe_core::papers::PaperFilter::ACTIVE,
        )
        .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    if let Some(raw) = uri.strip_prefix(PAPERS_PERIOD_PREFIX) {
        ...
        let today = griffe_core::clock::today_local();
        let checklist = griffe_core::papers::papers_checklist(store.connection(), period, today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        let papers = griffe_core::papers::list_papers(
            store.connection(),
    ...
    if uri == ASSETS_URI {
        ...
        let fy = fye.containing(griffe_core::clock::today_local());
        let assets = griffe_core::fixed_assets::list_fixed_assets(store.connection())
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(
            uri,
            assets
                .iter()
                .map(|a| griffe_core::fixed_assets::asset_json(a, fy))
                .collect::<Vec<_>>(),
        );
    }

    if uri == VAT_CARRY_URI {
        let carry = griffe_core::society::vat_carry_in(store.connection())
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(uri, carry);
    }

    if uri == OPENING_BALANCE_URI {
        let opening = griffe_core::opening_balance::opening_balance(store.connection())
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(
            uri,
            opening
    ...
    if let Some(period) = uri.strip_prefix(CLOSING_CHECKLIST_PREFIX) {
        ...
        let today = griffe_core::clock::today_local();
        let checklist = griffe_core::closing::closing_checklist(store.connection(), period, today)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(uri, griffe_core::closing::checklist_json(&checklist));
    }
    ...
    if let Some(period) = uri.strip_prefix(BALANCE_SHEET_PREFIX) {
        ...
        let (_, ledger) = griffe_core::ledger::ledger_ending_in(store.connection(), period)
            .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
        return json_contents(
            uri,
            griffe_core::ledger::balance_json(&ledger.trial_balance(), &ledger.balance_sheet()),
        );
    }
    if uri == COMPANY_URI {
        let today = griffe_core::clock::today_local();
        let profile =
            griffe_core::fiscal::company_profile_with_vat_filing(store.connection(), today)
                .map_err(|e| McpError::resource_not_found(e.to_string(), None))?;
    ...
    match schedule {
        ...
        missions::BillingSchedule::Forfait { installments } => json!({
            ...
            "installments": installments.iter().map(|(m, amount)| json!({
                ...
                "due_on": m.due_on.map(griffe_core::domain::format_date),
