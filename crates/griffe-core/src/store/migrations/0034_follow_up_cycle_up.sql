-- Une reprise de conversation pose un fait `cycle_opened`. SQLite ne sait pas
-- élargir un CHECK : on reconstruit la table.
...
CREATE TABLE follow_up_events (
    ...
    fact                TEXT NOT NULL CHECK (fact IN (
                            ...
                            'snoozed', 'date_set', 'retracted', 'cycle_opened'
                        )),
    at                  TEXT NOT NULL,
    until_on            TEXT,
