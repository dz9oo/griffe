...
//! `crate::clients` (lot 15) : succès → `200` corps vide + `HX-Trigger: griffe:saved` ; échec →
...
async fn current_opportunity(
    state: &AppState,
    id: OpportunityId,
) -> Option<Result<Option<griffe_core::domain::Opportunity>, AppError>> {
    state
...
fn parse_opportunity_form(
    ...
    let next_action_at = opt(&form.next_action)
        .map(|s| griffe_core::domain::parse_date(&s))
        .transpose()
        .ok()
        .flatten();
        ...
        Some(Ok(Some(o))) => {
            let (mut values, show_prospect) = state
                .with_store(|store| {
                    ...
                    let show_prospect = griffe_core::clients::prospect_party_is_editable(
                        store.connection(),
                        o.client_id,
                    )
                    .unwrap_or(false);
            ...
            values.next_action = o
                .next_action_at
                .map(griffe_core::domain::format_date)
                .unwrap_or_default();
...
pub async fn advance(
    ...
    let Ok(next_action_at) = griffe_core::domain::parse_date(&form.next_action) else {
...
pub async fn create_interaction(
    ...
    let occurred_at = opt(&form.occurred_on)
        .and_then(|s| griffe_core::domain::parse_date(&s).ok())
        .map(midnight_utc);
