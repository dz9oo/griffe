...
use griffe_cli::ControlPackReport;
use griffe_core::app::AppError;
use griffe_core::closing::StepStatus;
use griffe_core::domain::{PaperKind, format_date_fr};
use griffe_core::fiscal_year::list_fiscal_years;
...
pub fn chapter(
    ...
    let mut papers = griffe_core::papers::list_papers(
        store.connection(),
    ...
    let identity = griffe_core::papers::list_papers(
        store.connection(),
...
fn chapter_markup(
    ...
    html! {
        div class="letter" id="papiers-letter" data-view=(ViewId::Societe.slug())
            ...
            h1 { "Les papiers." }
            p class="lede" {
                "Les originaux nés ici restent ici. Ce que Griffe ne produit pas "
