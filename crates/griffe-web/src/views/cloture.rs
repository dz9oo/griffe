...
//! supprimer un projet, et télécharger les documents de clôture (`griffe-docs`). Depuis le
...
//! clôture » (`checklist_panel`) déroule les étapes calculées par `griffe_core::closing`, avec
...
use griffe_core::domain::{FiscalYearEnd, format_date};
use griffe_core::fec::{FecCheck, FecSeverity};
...
fn fiscal_year_end_of(store: &Store) -> FiscalYearEnd {
    griffe_core::company::company_profile(store.connection())
        .ok()
        .flatten()
...
fn euros_input_value(m: griffe_core::domain::Money) -> String {
...
/// La liste, avec son rafraîchissement automatique sur `griffe:saved` — même convention que
/// les autres écrans.
    ...
    Ok(html! {
        div id="cloture-list"
            ...
            div class="pipe-toolbar" {
                ...
                button class="btn" hx-get="/cloture/opening" hx-target="#panel" hx-swap="innerHTML" title="Reprise du dernier bilan tenu avant Griffe (expert-comptable)" { "bilan d'ouverture" }
...
const fn stage_badge_class(stage: griffe_core::closing::ClosingStage) -> &'static str {
    use griffe_core::closing::ClosingStage;
    match stage {
...
/// commande suggérée par la CLI (`griffe-cli::year::cli_hint`), branché sur la clé stable.
...
pub fn fec_check_panel(period: i32, check: &FecCheck) -> Markup {
    ...
    let debit = griffe_core::domain::Money::from_cents(check.total_debit_cents);
    let credit = griffe_core::domain::Money::from_cents(check.total_credit_cents);
    let body = html! {
        div class="detail-head" {
