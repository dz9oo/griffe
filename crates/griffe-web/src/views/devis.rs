...
use griffe_core::domain::{Discount, LineKind, Money, Quote, QuoteId, QuoteLine, QuoteStatus};
...
fn client_name(store: &Store, id: griffe_core::domain::ClientId) -> String {
    griffe_core::clients::list_clients(store.connection())
        .ok()
...
impl QuoteFormValues {
    ...
    pub fn from_quote(quote: &Quote) -> Self {
        ...
        Self {
            ...
            valid_until: griffe_core::domain::format_date(quote.valid_until),
        }
    }
}
...
pub fn detail_panel(
    ...
    let body = html! {
        ...
        dl class="detail-fields" {
            ...
            dt { "Valide jusqu'au" } dd { (griffe_core::domain::format_date(quote.valid_until)) }
...
pub fn list_fragment(store: &Store) -> Result<Markup, AppError> {
    ...
    let name_of = |id: griffe_core::domain::ClientId| {
        clients
            .iter()
            .find(|c| c.id == id)
                    ...
                    table {
                        ...
                        @for quote in &quotes {
                            tr class="row-clickable" hx-get=(format!("/devis/{}", quote.id)) hx-target="#panel" hx-swap="innerHTML" {
                                ...
                                td class="mono" style="padding-right:18px" { (griffe_core::domain::format_date(quote.valid_until)) }
                            }
                        }
                    }
                }
...
pub fn load(store: &Store, id: QuoteId) -> Result<Option<(Quote, QuoteReferences)>, AppError> {
    let Some(quote) = griffe_core::quotes::quote_by_id(store.connection(), id)? else {
        return Ok(None);
    };
