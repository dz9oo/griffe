...
use griffe_core::billing::{list_bank_transactions, unmatched_debits};
...
use griffe_core::expenses::{ExpenseDetail, expense_detail, list_expenses, reconciled_debits};
use griffe_core::fixed_assets::{expense_net, list_fixed_assets, should_be_immobilized};
            ...
            @if let Some((message, reload)) = &errors.conflict {
                ...
                @if values.bank_transaction_id.is_none() {
                    fieldset class="field" {
                        ...
                        (form::field_help("Sans ligne au relevé, Griffe fera comme si la banque avait payé ce jour-là. Si tu l'as payée de ta poche, choisis plutôt Moi."))
                        label class="field-checkbox" {
