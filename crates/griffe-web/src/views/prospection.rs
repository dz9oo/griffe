...
fn client_name(store: &Store, client_id: griffe_core::domain::ClientId) -> String {
...
fn first_contact(
    store: &Store,
    client_id: griffe_core::domain::ClientId,
) -> Option<griffe_core::domain::Contact> {
    griffe_core::clients::list_contacts(store.connection(), client_id)
        .ok()
...
fn party_address(
    store: &Store,
    client_id: griffe_core::domain::ClientId,
) -> Option<griffe_core::domain::Address> {
    list_clients(store.connection())
        .ok()
        .and_then(|clients| clients.into_iter().find(|c| c.id == client_id))
