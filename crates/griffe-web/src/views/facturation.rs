...
use griffe_core::billing::{compute_totals, list_invoices, list_payments, payments_for_invoice};
use griffe_core::clients::list_clients;
use griffe_core::domain::{Invoice, InvoiceId, Money, Payment, format_date};
...
pub fn load_detail(
    ...
    let Some(invoice) = griffe_core::billing::invoice_by_id(store.connection(), id)? else {
        return Ok(None);
    };
