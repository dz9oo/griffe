...
//! fiche de la facture, que l'utilisateur regarde encore) + `HX-Trigger: griffe:saved` pour
...
use griffe_core::domain::{InvoiceId, PaymentId};
use maud::html;
...
/// Fiche d'une facture (avec un bandeau d'erreur éventuel) + l'en-tête `griffe:saved` si la
...
async fn payment_or_not_found(
    state: &AppState,
    id: PaymentId,
) -> Result<griffe_core::domain::Payment, Box<Response>> {
    match state
