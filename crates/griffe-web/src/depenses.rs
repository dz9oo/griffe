...
//! `HX-Trigger: griffe:saved`, échec → `200` avec le panneau re-rendu. Toute action ici est un
...
//! reçu est archivé chiffré par `griffe_core::receipts::archive` — la même IO d'adaptateur que
...
//! `Command` ne touchent que `&Connection` »). Le corps du protocole `griffe://` arrive
//! entier en mémoire (voir `griffe-desktop/src/main.rs`) : le multipart y passe comme
...
use griffe_core::billing::{self, bank_transaction_by_id};
use griffe_core::domain::{
    AccountCode, BankTransactionId, ExpenseCategory, ExpenseId, FixedAssetId, Money, VatRate,
};
use griffe_core::expenses::{self, ExpenseDetail};
use griffe_core::fixed_assets::{self, AddFixedAsset, DeleteFixedAsset};
use maud::html;
...
async fn archive_uploaded(
    ...
    // la même implémentation que la CLI et le serveur MCP (`griffe_core::receipts`).
    let archived = state
        .with_store(|store| {
            griffe_core::receipts::archive(store, &receipt.filename, &receipt.content)
...
fn parse_expense_form(form: &ExpenseForm) -> Result<ParsedExpenseForm, Box<ExpenseFormErrors>> {
    ...
    let incurred_on = match griffe_core::domain::parse_date(form.incurred_on.trim()) {
        Ok(d) => Some(d),
        Err(e) => {
...
pub async fn create(State(state): State<AppState>, multipart: Multipart) -> Response {
    ...
    let paid_by = if bank_transaction_id.is_some() {
        griffe_core::domain::ExpensePaidBy::Company
    } else {
        match form.paid_by.as_deref() {
            Some("me") => griffe_core::domain::ExpensePaidBy::Associate,
            Some("company") => griffe_core::domain::ExpensePaidBy::Company,
            _ => {
                let errors = ExpenseFormErrors {
                    paid_by: Some(
    ...
    match execute(&state, cmd).await {
        ...
        Some(Ok(Outcome::Applied(id))) => {
            if let Some((filename, hash)) = captured_receipt.as_ref() {
                let _ = state
                    .with_store_mut(|store| {
                        griffe_cli::capture_expense_receipt(
                            store,
                            &AppState::human_ctx(),
                            id,
                            filename,
            ...
            if paid_by == griffe_core::domain::ExpensePaidBy::Associate {
...
async fn current_transaction(
    state: &AppState,
    id: griffe_core::domain::BankTransactionId,
) -> Option<Result<Option<griffe_core::domain::BankTransaction>, AppError>> {
    state
...
pub async fn settle_panel(State(state): State<AppState>, Path(id): Path<String>) -> Html<String> {
    let Some(id) = parse_id::<griffe_core::domain::BankTransactionId>(&id) else {
        return message_fragment("identifiant de transaction invalide");
    };
...
pub async fn settle(
    ...
    let account = match chosen.parse::<griffe_core::domain::SettlementAccount>() {
        Ok(a) => a,
        Err(e) => {
            return Html(
...
pub async fn receipt(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    ...
    let loaded = state
        .with_store(|store| -> Result<Option<(String, Vec<u8>)>, String> {
            ...
            let bytes = griffe_core::receipts::read(store, &filename).map_err(|e| e.to_string())?;
...
async fn expense_asset(state: &AppState, id: ExpenseId) -> Option<griffe_core::domain::FixedAsset> {
    state
