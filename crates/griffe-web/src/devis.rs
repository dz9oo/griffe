...
//! griffe:saved`, échec → `200` avec le panneau re-rendu (erreurs de champ ou bandeau). Voir
...
use griffe_core::domain::{Discount, Money, QuoteId, QuoteLine};
use griffe_core::quotes;
use maud::html;
...
fn parse_quote_content(form: &QuoteForm) -> Result<ParsedQuoteContent, Box<QuoteFormErrors>> {
    ...
    let valid_until = match griffe_core::domain::parse_date(form.valid_until.trim()) {
        Ok(d) => Some(d),
        Err(e) => {
...
pub async fn new_panel() -> Html<String> {
    ...
    let valid_until = griffe_core::clock::today_local() + time::Duration::days(30);
    let values = QuoteFormValues {
        valid_until: griffe_core::domain::format_date(valid_until),
        ..Default::default()
    };
...
pub async fn create(State(state): State<AppState>, Form(form): Form<QuoteForm>) -> Response {
    ...
    let opportunity_id = match opt(&form.opportunity) {
        ...
        Some(reference) => match state
            .with_store(|store| {
                griffe_core::reference::resolve_opportunity(store.connection(), &reference)
            })
            .await
        {
            None => return locked_fragment().into_response(),
            Some(Ok(griffe_core::reference::RefMatch::Unique(id))) => Some(id),
            Some(Ok(_)) | Some(Err(_)) => {
                return new_panel_with(QuoteFormErrors {
...
async fn quote_with_client_name(
    state: &AppState,
    id: QuoteId,
) -> Option<Result<Option<(griffe_core::domain::Quote, String)>, AppError>> {
    state
        .with_store(|store| {
            let Some(quote) = quotes::quote_by_id(store.connection(), id)? else {
                return Ok(None);
            };
            let client = griffe_core::clients::list_clients(store.connection())?
                .into_iter()
...
pub async fn accept_panel(State(state): State<AppState>, Path(id): Path<String>) -> Html<String> {
    ...
    match state
        .with_store(|store| griffe_core::quotes::quote_by_id(store.connection(), id))
        .await
    {
        None => locked_fragment(),
        Some(Err(e)) => message_fragment(&e.to_string()),
...
pub async fn accept(
    ...
    let started_on = match griffe_core::domain::parse_date(form.started_on.trim()) {
        Ok(d) => d,
        Err(e) => {
            let quote = match state
                .with_store(|store| griffe_core::quotes::quote_by_id(store.connection(), quote_id))
                .await
            {
                Some(Ok(Some(q))) => q,
