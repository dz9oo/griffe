...
use freeflow_core::clients::{ClientFilter, client_references, list_clients_with, list_contacts};
use freeflow_core::domain::{Client, ClientId, Contact};
...
pub fn detail_panel(
    client: &Client,
    contacts: &[Contact],
    refs: freeflow_core::clients::ClientReferences,
    error: Option<&str>,
) -> Markup {
    let body = html! {
...
/// lui-même après toute mutation (`freeflow:saved`, déclenché par `HX-Trigger` sur chaque
...
pub fn load_detail(
    ...
    Option<(
        Client,
        Vec<Contact>,
        freeflow_core::clients::ClientReferences,
    )>,
    AppError,
> {
    let Some(client) = freeflow_core::clients::client_by_id(store.connection(), id)? else {
        return Ok(None);
    };
