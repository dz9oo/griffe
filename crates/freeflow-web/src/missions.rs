...
use freeflow_core::domain::{Mission, MissionId, MissionKind, Money, TimeCategory, TimeEntryId};
...
pub async fn close(
    ...
    let Ok(ended_on) = freeflow_core::domain::parse_date(&form.ended_on) else {
...
pub async fn create_time_entry(
    ...
    let Ok(worked_on) = freeflow_core::domain::parse_date(&form.worked_on) else {
        let errors = TimeEntryFormErrors {
            banner: Some("date invalide".to_string()),
            ..Default::default()
        };
...
pub async fn update_time_entry(
    ...
    let worked_on = freeflow_core::domain::parse_date(&form.worked_on).unwrap_or(current.worked_on);
    let revision: i64 = form
        .revision
        .as_deref()
        .and_then(|s| s.parse().ok())
