...
//! - succès → `200` avec un corps **vide** et l'en-tête `HX-Trigger: freeflow:saved` — un swap
//!   `innerHTML` d'une chaîne vide vide le panneau (il se ferme), et le conteneur de liste
//!   (`views::clients::list_fragment`) écoute `hx-trigger="freeflow:saved from:body"` pour se
...
use freeflow_core::clients::{self, ClientFilter};
use freeflow_core::domain::{Address, Client, ClientId, ContactId, Siren, VatNumber};
use maud::html;
