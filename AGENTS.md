...
`direnv allow` puis `nix develop`. `just check` = fmt-check, clippy
`-D warnings`, machete, nextest, deny+audit. `nix flake check` avant de
dire « terminé ». Une invocation `nix develop -c …` longue peut être
signalée « killed » alors que le process continue : vérifier `ps -p`.
