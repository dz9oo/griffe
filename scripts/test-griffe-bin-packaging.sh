...
tauri_ver=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["version"])' \
  "$root/crates/griffe-desktop/tauri.conf.json")
[ -n "$pkgver" ] || fail "pkgver vide"
[ "$pkgver" = "$tauri_ver" ] || fail "pkgver PKGBUILD ($pkgver) ≠ tauri.conf ($tauri_ver)"
...
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
mkdir -p "$tmp/native/griffe-${pkgver}-x86_64-linux"
printf 'stub\n' > "$tmp/native/griffe-${pkgver}-x86_64-linux/griffe-desktop"
tar -C "$tmp/native" -cJf "$tmp/native/griffe-${pkgver}-x86_64-linux.tar.xz" \
  "griffe-${pkgver}-x86_64-linux"
want_sha=$(sha256sum "$tmp/native/griffe-${pkgver}-x86_64-linux.tar.xz" | awk '{print $1}')
...
test -f "$tmp/work/ci/griffe-${pkgver}-x86_64-linux.tar.xz" \
